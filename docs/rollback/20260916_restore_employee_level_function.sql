-- Captured from Production in a READ ONLY transaction, 2026-09-16.
-- Review and explicitly authorize before executing; replaces code only, not data.
begin;
CREATE OR REPLACE FUNCTION public.replace_employee_access_level(p_employee_id uuid, p_role_code text, p_expected_role text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
