...
export async function review(session, pieceId) {
  if (!/^collections-\d+-writer@example\.invalid$/.test(session.user?.email || '')) throw Error('Synthetic session required');
  ...
  const server=http.createServer((req,res)=>{
    ...
    if(pathname===entry&&!consumed){
      consumed=true;
      res.writeHead(200,{'Content-Type':'text/html'}).end(`<script>sessionStorage.setItem('museo-admin-environment','staging');localStorage.setItem('museo-admin-current-user','Catalogación ficticia');localStorage.setItem('museo-admin-access-level','Empleado');localStorage.setItem('museo-admin-supabase-session-staging',${JSON.stringify(JSON.stringify(session))});location.replace('/colecciones-museograficas.html?environment=staging');</script>`);
      return;
    }
