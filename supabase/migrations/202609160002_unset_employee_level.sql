-- Preserve the existing transactional RBAC checks; accept NULL expected only for an unset draft.
begin;
...
begin
  ...
  if requested is null or requested not in ('empleado','ejecutivo','administrador')
     or (expected is not null and expected not in ('empleado','ejecutivo','administrador')) then
  ...
  -- NULL expected is valid only for an unlinked, unset draft; the comparison
  -- still rejects concurrent assignments or a newly linked account.
  if expected is null and (employee_row.profile_id is not null or employee_row.access_level is not null) then
    -- A stale draft is an application conflict, not a transient serialization
    -- failure to retry. PostgREST forwards this as HTTP 409.
    raise exception 'ACCESS_LEVEL_CHANGED_RELOAD' using errcode='PT409';
  end if;
